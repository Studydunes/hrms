...
class EmpController extends Controller
{
    ...
    public function processEmployee(Request $request)
    {
        ...
        // $user->email    = str_replace(' ', '_', $request->mobile_phone).'@soarlogic.com';
        ...
        $user = User::where('id',$emp_id)->with('employee','role.role')->first();
        $emp_role = $user->role->role->name;

        if($emp_role == 'Team Lead'||$emp_role == 'IT Executive' || $emp_role == 'Software Developer' || $emp_role == 'Senior Software Developer') {
